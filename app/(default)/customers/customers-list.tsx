...
import { getCustomerLinks } from "@/content/queries";
...
export default async function CustomersList() {
  ...
  const data = await getCustomerLinks();
  const customerItems = data.customerPostCollection.items;
    ...
    <div className="max-w-[352px] mx-auto sm:max-w-[728px] lg:max-w-none pb-12 md:pb-20">
      <div className="my-20 flex justify-center gap-5">
        {customerItems.map((item) => (
          <div key={item.slug}>
            <Link href={`/customers/${item.slug}`}>
              <span className="inline-flex items-center justify-center p-2 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-full shadow-lg hover:shadow-xl transition duration-300 ease-in-out">
                <Image
                  src={item.customer.logo.url}
                  width={24}
                  height={24}
                  alt={item.customer.logo.title}
                />
