...
import { getCustomerLinks, getCustomerPost } from "@/content/queries";
...
export const generateStaticParams = async () => {
  ...
  return data.customerPostCollection.items.map((item) => ({ slug: item.slug }));
};

export default async function CustomerSingle({ params }: { params: Params }) {
  const data = await getCustomerPost(params.slug);
  const { title, customer, body } = data.customerPostCollection.items[0];
  return (
